...
flowchart LR
    A["🔄 Protocol Activity\n(Trades & Volume)"]
    B["💰 Fee Revenue"]
    C["🏛️ On-Chain Treasury"]
    D["🔒 Staking & Bonds\n(Merchants & Verifiers)"]
    E["🗳️ Token Governance\n(Futarchy-based)"]
    F["📈 Token Value\n(Holders)"]
    G["🛡️ Ecosystem Grants\n& Audits"]

    A --> B
    B --> C
    B -->|"Rebates & discounts"| F
    C --> G
    C --> E
    D -->|"Honest behavior\nalignment"| A
    E -->|"Parameters, upgrades,\nfee routing"| A
    F -->|"Stake as bonds"| D
```
...
| --------------- | ---------------------------------------------------- |
| **Ownership**   | IP, treasury, and mint rights controlled by holders  |
| **Governance**  | Futarchy-based — market predicts then votes          |
| **Staking**     | Merchants & verifiers post bonds for honest behavior |
| **Fee Rebates** | Active participants earn discounts on protocol fees  |
| **Insurance**   | Dispute insurance pool participation                 |
