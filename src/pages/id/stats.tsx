...
function RegionCard({ circle, maxOrders }: RegionCardProps) {
    ...
    const meta = CIRCLE_META[decodedCurrency] ?? {
        flag: "\u{1F310}",
        country: decodedCurrency || "Tidak Diketahui",
        color: "#493fee",
    };
        ...
        <div className={styles.regionCard}>
            ...
            <div className={styles.regionMetrics}>
                <div className={styles.regionMetric}>
                    ...
                    <span className={styles.regionMetricLabel}>Sengketa</span>
                </div>
                <div className={styles.regionMetric}>
                    <span className={styles.regionMetricValue}>{formatUSDC(circle.metrics.totalVolume)}</span>
                    <span className={styles.regionMetricLabel}>Volume Bulanan</span>
                </div>
                <div className={styles.regionMetric}>
                    <span className={styles.regionMetricValue}>{resolutionRate}%</span>
                    <span className={styles.regionMetricLabel}>Penyelesaian</span>
                </div>
            </div>

            <div className={styles.progressBar}>
                <div className={styles.progressLabel}>
                    <span>Pangsa Volume</span>
...
export default function StatsPage(): JSX.Element {
    ...
    useEffect(() => {
        async function fetchStats() {
            try {
                ...
                setError(
                    "Statistik langsung tidak dapat dimuat. Jaringan mungkin sedang tidak tersedia untuk sementara."
                );
            } finally {
                setLoading(false);
            }
    ...
    return (
        <Layout
            ...
            description="Statistik on-chain langsung dari P2P Protocol — total transaksi, merchant aktif, negara, dan kesehatan jaringan."
        >
            <div className={styles.page}>
                {/* ── Hero ── */}
                <section className={styles.hero}>
                    <div className={styles.heroInner}>
                        <div className={styles.heroTag}>Data On-Chain Langsung</div>
                        <h1 className={styles.heroTitle}>Statistik Protokol</h1>
                        <p className={styles.heroSubtitle}>
                            Transparansi real-time tentang jangkauan global, volume,
                            dan kesehatan jaringan P2P Protocol. Semua data sesuai dengan Ops Dashboard langsung.
                        </p>
                        {lastUpdated && (
                ...
                <div className={styles.content}>
                    ...
                    <section className={styles.section}>
                        <h2 className={styles.sectionTitle}>Ringkasan Global</h2>
                        <p className={styles.sectionSubtitle}>
                            Metrik gabungan dari semua wilayah aktif protokol.
                        </p>
                        ...
                        {loading ? (
                            ...
                            <div className={styles.statsGrid}>
                                <StatCard
                                    ...
                                    value={totalOrders}
                                    description="Pesanan yang diselesaikan di semua wilayah (Bulan berjalan)"
                                />
                                <StatCard
                                    icon="🏪"
                                    label="Merchant Aktif"
                                    value={totalMerchants}
                                    description="Merchant terverifikasi yang menyediakan likuiditas"
                                />
                                <StatCard
                                    icon="💰"
                                    ...
                                    description="Total volume di semua wilayah bulan ini"
                                />
                                <StatCard
                                    icon="🌍"
                                    label="Negara Aktif"
                                    value={consolidatedCircles.length}
                                    description="Lingkaran mata uang regional yang berbeda"
                                />
                            </div>
                        )}
                    </section>
                    ...
                    {!loading && !error && (
                        <section className={styles.section}>
                            <h2 className={styles.sectionTitle}>Arti Angka-Angka Ini</h2>
                            <div className={styles.explainerGrid}>
                                <div className={styles.explainerCard}>
                                    <div className={styles.explainerIcon}>🔄</div>
                                    <h3 className={styles.explainerTitle}>Pesanan Bulanan</h3>
                                    <p className={styles.explainerText}>
                                        Setiap pesanan mewakili seseorang yang menukarkan mata uang
                                        fiat dengan kripto (atau sebaliknya) melalui protokol. Angka-angka
                                        ini menunjukkan penggunaan aktif sepanjang bulan kalender berjalan.
                                    </p>
                                </div>
                                <div className={styles.explainerCard}>
                                    <div className={styles.explainerIcon}>🏪</div>
                                    <h3 className={styles.explainerTitle}>Merchant</h3>
                                    <p className={styles.explainerText}>
                                        Merchant adalah "penyedia likuiditas" protokol —
                                        mereka memegang USDC dan menukarkannya dengan mata uang lokal
                                        di kedua sisi transaksi. Lebih banyak merchant berarti penyelesaian
                                        lebih cepat dan tarif lebih baik bagi pengguna.
                                    </p>
                                </div>
                                <div className={styles.explainerCard}>
                                    <div className={styles.explainerIcon}>🌍</div>
                                    <h3 className={styles.explainerTitle}>Lingkaran per Negara</h3>
                                    <p className={styles.explainerText}>
                                        Setiap negara beroperasi dalam "Lingkaran" tersendiri — pasar
                                        terisolasi dengan aturan mata uang lokal, kumpulan merchant, dan
                                        harga masing-masing. Desain ini memungkinkan P2P berkembang ke
                                        negara baru tanpa memengaruhi pasar yang sudah ada.
                                    </p>
                                </div>
                                <div className={styles.explainerCard}>
                                    <div className={styles.explainerIcon}>🛡️</div>
                                    <h3 className={styles.explainerTitle}>Total Sengketa</h3>
                                    <p className={styles.explainerText}>
                                        Jumlah mutlak transaksi yang memerlukan arbitrase protokol.
                                        Mengingat ribuan pesanan yang ditempatkan setiap bulan,
                                        menjaga jumlah sengketa mendekati nol merupakan bukti
                                        keandalan alur protokol.
                                    </p>
                                </div>
                            </div>
                        </section>
                    ...
                    {!loading && !error && sortedRegions.length > 0 && (
                        <section className={styles.section}>
                            <h2 className={styles.sectionTitle}>Rincian Regional</h2>
                            <p className={styles.sectionSubtitle}>
                                Setiap kartu menampilkan cuplikan langsung aktivitas protokol di suatu negara untuk bulan berjalan.
                                Bilah kemajuan menunjukkan pangsa volume dari total global.
                            </p>
                    ...
                    <section className={styles.sourceSection}>
                        <div className={styles.sourceCard}>
                            <div className={styles.sourceLeft}>
                                <div className={styles.sourceTitle}>Sumber Data dan Transparansi</div>
                                <p className={styles.sourceText}>
                                    Semua statistik di halaman ini diambil secara langsung dari
                                    smart contract on-chain yang diindeks oleh The Graph Protocol.
                                    Ini berarti data tidak dapat dimanipulasi. Ini adalah
                                    pembacaan langsung dari blockchain.
                                </p>
                            </div>
